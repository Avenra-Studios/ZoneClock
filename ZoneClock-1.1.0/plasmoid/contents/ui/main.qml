...
PlasmoidItem {
    ...
    fullRepresentation: Item {
        ...
        // Responsive sizing: calculate dimensions based on screen resolution
        // to prevent content from being cut off on different displays
        property int screenWidth: Screen.width
        property int screenHeight: Screen.height
        
        // Use 70% of screen width/height, but with sensible min/max bounds
        property int baseWidth: Math.max(
            Kirigami.Units.gridUnit * 32,
            Math.min(Kirigami.Units.gridUnit * 50, Math.round(screenWidth * 0.7))
        )
        property int baseHeight: Math.max(
            Kirigami.Units.gridUnit * 40,
            Math.min(Kirigami.Units.gridUnit * 60, Math.round(screenHeight * 0.7))
        )

        readonly property int popupWidth: baseWidth
        readonly property int popupHeight: baseHeight
        ...
        Layout.minimumHeight: Kirigami.Units.gridUnit * 35
            ...
            QQC2.ScrollView {
                ...
                ColumnLayout {
                    ...
                    Repeater {
                        ...
                        onCountChanged: {
                            timezoneList.Layout.preferredHeight = undefined
                            timezoneList.forceLayout()
                        }
                    ...
                    Item {
                        Layout.fillWidth: true
                        Layout.fillHeight: true
                        visible: root.timezoneData.length === 0

                        PlasmaComponents.Label {
                            anchors.centerIn: parent
                            text: i18n("No timezones added yet")
                            opacity: 0.6
                        }
                    }
    ...
    // Plasma auto-hides an applet's expanded popup the instant a
    // different window becomes active - including the Add/Edit
    // dialog this widget just opened itself, since it's a separate
    // top-level Window. Without this, clicking "Edit" (or "Add
    // Timezone") makes the whole popup collapse right as the dialog
    // opens, which looks like the button does nothing (or the popup
    // flashes/collapses mid-layout, scrambling its contents).
    // Suppress that auto-hide only while one of our own dialogs is
    // open.
    Binding {
        target: plasmoid
        property: "hideOnWindowDeactivate"
        value: !(addDialog.visible || editDialog.visible)
    }
}
